...
pub fn report_update_failure(
    ...
    report.insert("previous_version".to_string(), attempt.previous_version.clone());
    ...
    if telemetry.include_system_info {
        ...
        report.insert("architecture".to_string(), std::env::consts::ARCH.to_string());
...
fn create_error_report_issue(report: &HashMap<String, String>) -> Result<()> {
    ...
    let title = format!(
        "Update failure: {} -> {}",
        report.get("previous_version").unwrap_or(&"unknown".to_string()),
    ...
    body.push_str("```\n");
    body.push_str(report.get("error_chain").unwrap_or(&"unavailable".to_string()));
    body.push_str("\n```\n</details>\n");
    
    // create issue with labels
    client.create_issue(&title, &body, vec!["bug", "auto-reported", "update-failure"])?;
